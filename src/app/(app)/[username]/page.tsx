            ...
            <div className="bg-muted/30 rounded-3xl p-6 border border-border">
              ...
              <div className="flex flex-wrap gap-3">
                {socialLinks.facebook && (
                  <button onClick={() => openSocialLink(socialLinks.facebook!)} className="w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center text-white hover:scale-110 transition-transform">
                    <FaFacebook className="w-6 h-6" />
                  </button>
                )}
                {socialLinks.instagram && (
                  <button onClick={() => openSocialLink(socialLinks.instagram!)} className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 flex items-center justify-center text-white hover:scale-110 transition-transform">
                    <FaInstagram className="w-6 h-6" />
                  </button>
                )}
                {socialLinks.twitter && (
                  <button onClick={() => openSocialLink(socialLinks.twitter!)} className="w-12 h-12 rounded-full bg-black flex items-center justify-center text-white hover:scale-110 transition-transform">
                    <FaTwitter className="w-6 h-6" />
                  </button>
                )}
                {socialLinks.linkedin && (
                  <button onClick={() => openSocialLink(socialLinks.linkedin!)} className="w-12 h-12 rounded-full bg-blue-700 flex items-center justify-center text-white hover:scale-110 transition-transform">
                    <FaLinkedin className="w-6 h-6" />
                  </button>
                )}
                {socialLinks.github && (
                  <button onClick={() => openSocialLink(socialLinks.github!)} className="w-12 h-12 rounded-full bg-gray-800 flex items-center justify-center text-white hover:scale-110 transition-transform">
                    <FaGithub className="w-6 h-6" />
                  </button>
                )}
                {socialLinks.tiktok && (
                  <button onClick={() => openSocialLink(socialLinks.tiktok!)} className="w-12 h-12 rounded-full bg-black flex items-center justify-center text-white hover:scale-110 transition-transform">
                    <FaTiktok className="w-6 h-6" />
                  </button>
                )}
                {socialLinks.youtube && (
                  <button onClick={() => openSocialLink(socialLinks.youtube!)} className="w-12 h-12 rounded-full bg-red-600 flex items-center justify-center text-white hover:scale-110 transition-transform">
                    <FaYoutube className="w-6 h-6" />
                  </button>
                )}
                {socialLinks.website && (
                  <button onClick={() => openSocialLink(socialLinks.website!)} className="w-12 h-12 rounded-full bg-green-600 flex items-center justify-center text-white hover:scale-110 transition-transform">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end sm:items-center justify-center">
          <div className="bg-background w-full max-w-lg max-h-[90vh] flex flex-col rounded-t-3xl sm:rounded-3xl animate-in slide-in-from-bottom-4 shadow-2xl">
            <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-background/80 backdrop-blur-md sticky top-0 z-10">
              ...
              <button onClick={() => setShowEditModal(false)} className="p-2 rounded-full hover:bg-muted transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
                  ...
                  {[
                    ...
                    <div key={key} className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        onChange={(e) => setEditForm({ ...editForm, social_links: { ...editForm.social_links, [key]: e.target.value } })}
            ...
            <div className="p-6 border-t border-border bg-muted/10 flex gap-3">
              <button onClick={() => setShowEditModal(false)} className="flex-1 py-4 rounded-full border-2 border-border text-muted-foreground font-black text-[10px] uppercase tracking-widest transition-all active:scale-95">
                Cancel
              </button>
              <button onClick={saveProfile} disabled={saving} className="flex-1 py-4 rounded-full bg-green-600 text-white font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 hover:bg-green-700 shadow-lg shadow-green-600/20 disabled:opacity-50">
