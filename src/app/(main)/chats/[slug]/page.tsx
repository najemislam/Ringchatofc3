...
import { ChevronLeft, Send, Trash2, MoreVertical, Heart, X, Ban, Trash, Smile, Phone, PhoneOff, Video, Loader2 } from "lucide-react"
...
export default function ChatSlugPage() {
  ...
  // Resolve slug: if UUID → use directly, if username → find/create conversation
  useEffect(() => {
    ...
    const resolve = async () => {
      ...
      if (myParticipants) {
        for (const conv of myParticipants) {
          ...
          if (other) {
            setResolvedConvId(conv.conversation_id)
            setResolving(false)
            return
          }
        }
  ...
  return <ChatView conversationId={resolvedConvId} />
}
...
function ChatView({ conversationId }: { conversationId: string }) {
  ...
  const blockUser = async () => {
    ...
    await supabase
      ...
      .delete()
      .or(
        `and(user_id.eq.${user.id},friend_id.eq.${otherUser.id}),and(user_id.eq.${otherUser.id},friend_id.eq.${user.id})`
      )
                ...
                >
                  ...
                  {reactionMenuMessageId === msg.id && (
                    <>
                      ...
                      />
                      <div
                        className={`absolute z-[70] -top-12 ${
                          isMe ? "right-0" : "left-0"
                        } bg-background border border-border rounded-full shadow-xl px-2 py-1.5 flex items-center gap-2 animate-in fade-in zoom-in duration-200`}
                      >
                        {["👍", "❤️", "😊", "🙁", "😠"].map((emoji) => (
                          <button
                            key={emoji}
                  ...
                  {selectedMessageId === msg.id && (
                    <p
                      className={`text-[10px] mt-1 mb-1 px-2 text-muted-foreground animate-in fade-in slide-in-from-top-1 duration-200`}
                    >
