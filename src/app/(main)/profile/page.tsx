    ...
    <div className="flex flex-col min-h-screen bg-background pb-24">
      ...
      <div className="flex items-center justify-between px-2 pt-2 pb-2 bg-background border-b border-border">
        ...
        <div className="flex items-center gap-1">
          {isOwnProfile && (
            <Link href="/search" className="w-10 h-10 flex items-center justify-center rounded-full text-foreground active:scale-95 transition-transform hover:bg-muted">
              <Search className="w-5 h-5" strokeWidth={2} />
            </Link>
          )}
          
          {isOwnProfile ? (
            <Link href="/settings" className="w-10 h-10 flex items-center justify-center rounded-full text-foreground active:scale-95 transition-transform hover:bg-muted">
        ...
        <div className="p-6 space-y-6">
          {profileUser.bio && (
            <div className="bg-muted/30 rounded-3xl p-6 border border-border">
              ...
              <p className="text-sm font-medium leading-relaxed">{profileUser.bio}</p>
            </div>
          )}
              ...
              {profileUser.date_of_birth && (
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-base font-bold font-sans">
                    {new Date(profileUser.date_of_birth).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                  </p>
                </div>
              )}
              {profileUser.gender && (
                <div className="flex items-center gap-3">
                  <span className="text-xl flex-shrink-0">{profileUser.gender.toLowerCase() === 'male' ? '♂️' : '♀️'}</span>
                ...
                {profileUser.address && (
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium mt-1">{COUNTRIES.find(c => c.name === profileUser.address)?.flag || ""} {profileUser.address}</p>
                    </div>
                  </div>
                )}
                {profileUser.phone && (
          ...
          {Object.values(socialLinks).some(v => v) && (
            <div className="bg-muted/30 rounded-3xl p-6 border border-border">
              ...
              <div className="flex flex-wrap gap-3">
                {socialLinks.facebook && <button onClick={() => openSocialLink(socialLinks.facebook!)} className="w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center text-white hover:scale-110 transition-transform"><FaFacebook className="w-6 h-6" /></button>}
                {socialLinks.instagram && <button onClick={() => openSocialLink(socialLinks.instagram!)} className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 flex items-center justify-center text-white hover:scale-110 transition-transform"><FaInstagram className="w-6 h-6" /></button>}
                {socialLinks.twitter && <button onClick={() => openSocialLink(socialLinks.twitter!)} className="w-12 h-12 rounded-full bg-black flex items-center justify-center text-white hover:scale-110 transition-transform"><FaTwitter className="w-6 h-6" /></button>}
                {socialLinks.linkedin && <button onClick={() => openSocialLink(socialLinks.linkedin!)} className="w-12 h-12 rounded-full bg-blue-700 flex items-center justify-center text-white hover:scale-110 transition-transform"><FaLinkedin className="w-6 h-6" /></button>}
                {socialLinks.github && <button onClick={() => openSocialLink(socialLinks.github!)} className="w-12 h-12 rounded-full bg-gray-800 flex items-center justify-center text-white hover:scale-110 transition-transform"><FaGithub className="w-6 h-6" /></button>}
                {socialLinks.tiktok && <button onClick={() => openSocialLink(socialLinks.tiktok!)} className="w-12 h-12 rounded-full bg-black flex items-center justify-center text-white hover:scale-110 transition-transform"><FaTiktok className="w-6 h-6" /></button>}
                {socialLinks.youtube && <button onClick={() => openSocialLink(socialLinks.youtube!)} className="w-12 h-12 rounded-full bg-red-600 flex items-center justify-center text-white hover:scale-110 transition-transform"><FaYoutube className="w-6 h-6" /></button>}
                {socialLinks.website && <button onClick={() => openSocialLink(socialLinks.website!)} className="w-12 h-12 rounded-full bg-green-600 flex items-center justify-center text-white hover:scale-110 transition-transform"><Globe className="w-6 h-6" /></button>}
              </div>
            </div>
          )}
          ...
          {!profileUser.address && !profileUser.phone && !profileUser.email && !Object.values(socialLinks).some(v => v) && (
            <div className="flex flex-col items-center justify-center py-12 text-center opacity-30 gap-2">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest">No information added yet</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end sm:items-center justify-center">
          <div className="bg-background w-full max-w-lg max-h-[90vh] flex flex-col rounded-t-3xl sm:rounded-3xl animate-in slide-in-from-bottom-4 shadow-2xl">
            <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-background/80 backdrop-blur-md sticky top-0 z-10">
              ...
              <button onClick={() => setShowEditModal(false)} className="p-2 rounded-full hover:bg-muted transition-colors"><X className="w-5 h-5" /></button>
            </div>
            
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="flex flex-col items-center">
                <div className="relative group">
                  <div className="w-24 h-24 rounded-full bg-muted flex items-center justify-center overflow-hidden border-4 border-green-600 cursor-pointer shadow-xl transition-transform active:scale-95" onClick={() => fileInputRef.current?.click()}>
                    {profileUser.profile_picture ? <img src={profileUser.profile_picture} alt="" className="w-full h-full object-cover" /> : <span className="text-3xl font-black text-green-600/30">{profileUser.full_name.charAt(0)}</span>}
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity rounded-full"><Upload className="w-6 h-6 text-white" /></div>
                  </div>
                </div>
              ...
              <div>
                ...
                <input type="text" value={editForm.full_name} onChange={(e) => setEditForm({ ...editForm, full_name: e.target.value })} className="w-full mt-2 px-4 py-3 rounded-xl border border-border bg-muted/50 focus:ring-2 focus:ring-green-500 outline-none text-sm font-medium" />
              </div>

              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Bio</label>
                <textarea value={editForm.bio} onChange={(e) => setEditForm({ ...editForm, bio: e.target.value })} placeholder="Write something about yourself..." className="w-full mt-2 px-4 py-3 rounded-xl border border-border bg-muted/50 resize-none focus:ring-2 focus:ring-green-500 outline-none text-sm font-medium" rows={3} maxLength={150} />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Gender</label>
                  <select value={editForm.gender} onChange={(e) => setEditForm({ ...editForm, gender: e.target.value })} className="w-full mt-2 px-4 py-3 rounded-xl border border-border bg-muted/50 focus:ring-2 focus:ring-green-500 outline-none text-sm font-medium appearance-none cursor-pointer">
                ...
                <div>
                  ...
                  <input type="date" value={editForm.date_of_birth} onChange={(e) => setEditForm({ ...editForm, date_of_birth: e.target.value })} className="w-full mt-2 px-4 py-3 rounded-xl border border-border bg-muted/50 focus:ring-2 focus:ring-green-500 outline-none text-sm font-medium" />
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-border">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select value={editForm.country} onChange={(e) => setEditForm({ ...editForm, country: e.target.value })} className="w-full mt-2 px-4 py-3 rounded-xl border border-border bg-muted/50 focus:ring-2 focus:ring-green-500 outline-none text-sm font-medium appearance-none cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-2">
                      <div className="px-4 py-3 rounded-xl border border-border bg-muted/30 text-sm font-bold text-muted-foreground min-w-[80px] text-center">{editForm.country ? COUNTRIES.find(c => c.name === editForm.country)?.code || "+00" : "+00"}</div>
                      <input type="tel" value={editForm.phone} onChange={(e) => setEditForm({ ...editForm, phone: e.target.value })} placeholder="Phone number" className="flex-1 px-4 py-3 rounded-xl border border-border bg-muted/50 focus:ring-2 focus:ring-green-500 outline-none text-sm font-medium" />
                    </div>
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Email</label>
                    <input type="email" value={editForm.email} onChange={(e) => setEditForm({ ...editForm, email: e.target.value })} placeholder="Your email address" className="w-full mt-2 px-4 py-3 rounded-xl border border-border bg-muted/50 focus:ring-2 focus:ring-green-500 outline-none text-sm font-medium" />
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-border">
                ...
                <div className="space-y-4">
                  {[
                    { key: 'facebook', icon: <FaFacebook className="w-5 h-5" />, bg: 'bg-blue-600', placeholder: 'Facebook URL' },
                    { key: 'instagram', icon: <FaInstagram className="w-5 h-5" />, bg: 'bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400', placeholder: 'Instagram URL' },
                    { key: 'twitter', icon: <FaTwitter className="w-5 h-5" />, bg: 'bg-black', placeholder: 'Twitter/X URL' },
                    { key: 'linkedin', icon: <FaLinkedin className="w-5 h-5" />, bg: 'bg-blue-700', placeholder: 'LinkedIn URL' },
                    { key: 'github', icon: <FaGithub className="w-5 h-5" />, bg: 'bg-gray-800', placeholder: 'GitHub URL' },
                    { key: 'tiktok', icon: <FaTiktok className="w-5 h-5" />, bg: 'bg-black', placeholder: 'TikTok URL' },
                    { key: 'youtube', icon: <FaYoutube className="w-5 h-5" />, bg: 'bg-red-600', placeholder: 'YouTube URL' },
                    { key: 'website', icon: <Globe className="w-5 h-5" />, bg: 'bg-green-600', placeholder: 'Website URL' },
                  ].map(({ key, icon, bg, placeholder }) => (
                    <div key={key} className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full ${bg} flex items-center justify-center text-white flex-shrink-0`}>{icon}</div>
                      <input type="url" value={(editForm.social_links as any)[key] || ""} onChange={(e) => setEditForm({ ...editForm, social_links: { ...editForm.social_links, [key]: e.target.value } })} placeholder={placeholder} className="flex-1 px-4 py-3 rounded-xl border border-border bg-muted/50 focus:ring-2 focus:ring-green-500 outline-none text-sm font-medium" />
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="p-6 border-t border-border bg-muted/10 flex gap-3">
              <button onClick={() => setShowEditModal(false)} className="flex-1 py-4 rounded-full border-2 border-border text-muted-foreground font-black text-[10px] uppercase tracking-widest transition-all active:scale-95">Cancel</button>
              <button onClick={saveProfile} disabled={saving} className="flex-1 py-4 rounded-full bg-green-600 text-white font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 hover:bg-green-700 shadow-lg shadow-green-600/20 disabled:opacity-50">{saving ? "Saving..." : "Save Changes"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <AlertDialog open={showBlockDialog} onOpenChange={setShowBlockDialog}>
        <AlertDialogContent className="max-w-sm mx-auto rounded-3xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-sm text-muted-foreground">Choose how you want to block this user</AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-3 py-4">
            <button
              onClick={async () => {
                ...
                setIsBlocked(true); setBlockType('full'); setIsFollowing(false); setIsFriend(false); setShowBlockDialog(false)
                toast.success("User completely blocked")
              }}
              className="w-full p-4 rounded-2xl border-2 border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-900 text-left hover:border-red-500 transition-all"
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-red-500 flex items-center justify-center text-white"><Ban className="w-5 h-5" /></div>
                <div className="flex-1">
                  <p className="font-bold text-red-600 dark:text-red-400">Complete Block</p>
                  <p className="text-xs text-muted-foreground mt-0.5">Block everything - messages, interactions, profile view</p>
                </div>
              </div>
            </button>
            <button
              onClick={async () => {
                ...
                setIsBlocked(true); setBlockType('partial'); setIsFriend(false); setShowBlockDialog(false)
                toast.success("User interaction blocked")
              }}
              className="w-full p-4 rounded-2xl border-2 border-orange-200 bg-orange-50 dark:bg-orange-950/20 dark:border-orange-900 text-left hover:border-orange-500 transition-all"
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-orange-500 flex items-center justify-center text-white"><MessageCircle className="w-5 h-5" /></div>
                <div className="flex-1">
                  <p className="font-bold text-orange-600 dark:text-orange-400">Interaction Block</p>
                  <p className="text-xs text-muted-foreground mt-0.5">Block messages and reactions but can view profile</p>
                </div>
              </div>
            </button>
          </div>
